...
def continue_mpc_validator(request_data, self_db_data, shares_count, processed_request_data, for_ubic):
...
def finalize_mpc_validator(request_data, self_db_data, shares_count, processed_request_data):
...
def test_mpc():
    ...
    for self_db_data in self_db_data_sets:
        shares_count = len(self_db_data[drivers_hash_ids[0]])  # lens should be equal
        request_data = get_request_data(drivers_hash_ids, shares_count)

        continue_mpc_helper(request_data, self_db_data, shares_count)
        finalize_mpc_helper(request_data, self_db_data, shares_count)
        for endpoint_hash_id in endpoint_hash_ids:
            original_request_data = deepcopy(request_data)
            for_ubic, request_data = compute(request_data, self_db_data, endpoint_hash_id)
            if endpoint_hash_id:
                continue_mpc_validator(original_request_data, self_db_data, shares_count, request_data, for_ubic)
            else:
                finalize_mpc_validator(original_request_data, self_db_data, shares_count, request_data)
